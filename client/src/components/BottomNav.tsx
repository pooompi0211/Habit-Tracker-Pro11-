...
export function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-lg border-t border-border/50 pb-safe">
          ...
          return (
            <Link key={item.href} href={item.href}>
              <div className={cn(
                ...
                />
                <span className={cn("text-[10px] font-medium", isActive ? "opacity-100" : "opacity-0 scale-0 h-0 overflow-hidden")}>
                  {item.label}
                </span>
              </div>
            </Link>
